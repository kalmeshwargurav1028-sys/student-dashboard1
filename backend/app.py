...
_PROJECT_ROOT = (
    ...
    if os.path.basename(_THIS_DIR) == 'backend'
    else _THIS_DIR
)
...
def configure_elevenlabs():
    try:
        config_data = (db.settings.find_one({}, {'_id': 0}) if db is not None else {}) or {}
        api_key = os.environ.get('ELEVENLABS_API_KEY') or config_data.get('ELEVENLABS_API_KEY')
...
if not _secret_key:
    ...
    if os.environ.get('VERCEL'):
        warnings.warn(
            "SECRET_KEY is not set on Vercel. Set it in Project Settings → Environment Variables. "
            "Using a derived fallback so sessions stay stable across cold starts.",
            stacklevel=2
        )
        _secret_key = hashlib.sha256((os.environ.get('MONGO_URI') or 'vercel').encode()).hexdigest()
    else:
        warnings.warn(
            "SECRET_KEY environment variable is not set. "
            "Using a random key for this process only — set SECRET_KEY in your .env file.",
            stacklevel=2
        )
...
def _password_is_hashed(stored):
    return isinstance(stored, str) and stored.startswith(('pbkdf2:', 'scrypt:', 'argon2:'))

def verify_password(stored, provided):
    """Check a password against a hashed or legacy plaintext value."""
    if not stored or provided is None or provided == '':
        return False
...
def upgrade_password_if_plaintext(collection, query, stored, provided):
    """Re-hash a legacy plaintext password after a successful login."""
    if stored and provided and not _password_is_hashed(stored) and stored == provided:
        try:
            collection.update_one(query, {'$set': {'password': generate_password_hash(provided)}})
        except Exception:
            pass
...
def ensure_bootstrap_admin():
    ...
    if not email or not password:
        if os.environ.get('VERCEL'):
            print('[AUTH] No admins exist. Set ADMIN_EMAIL and ADMIN_PASSWORD in Vercel env to seed the first admin.')
            return
        email = email or 'admin@localhost'
        password = password or 'ChangeMeNow!'
        print(f'[AUTH] Seeded local admin {email}. Change this password immediately.')
...
# Credentials come from .env or Vercel environment variables, never from source.
# ---------------------------------------------------------------------------
...
def admin_dashboard():
    ...
    # Process students
    student_users = list(db.student_users.find({}, {'password': 0}))
    student_profiles = {s['id']: s for s in db.students.find(get_student_query(), {'_id': 0, 'password': 0})}
...
def utility_refresh_data():
    ...
    def _snapshot():
        ...
        return {
            ...
            'by': session.get('username') or 'Admin',
            'total': sum(int(b.get('count') or 0) for b in banks),
            'counts': {b['key']: int(b.get('count') or 0) for b in banks},
            'labels': {b['key']: b['label'] for b in banks},
        }

    if request.method == 'POST':
        snap = _snapshot()
        history = list(settings.get('refresh_history') or [])
        history.insert(0, snap)
        history = history[:8]
        db.settings.update_one(
            {},
            {'$set': {
                'last_data_refresh': snap['at'],
                'last_data_refresh_by': snap['by'],
                'last_data_refresh_total': snap['total'],
        ...
        log_notification(
            'Data refresh',
            f"{snap['by']} refreshed portal data ({snap['total']} records counted).",
            role_target='admin',
        )
        flash('Portal lists reloaded from the database. No records were deleted.')
        return redirect(url_for('utility_refresh_data'))

    banks = _cache_monitor_banks()
    total_records = sum(int(b.get('count') or 0) for b in banks)
    last_refresh = settings.get('last_data_refresh')
    last_by = settings.get('last_data_refresh_by')
    age_hours = None
    freshness = 'unknown'
    last_friendly = None
    if last_refresh:
        try:
            ...
            age_sec = max((datetime.utcnow() - refreshed_at).total_seconds(), 0)
            ...
            if age_hours < 6:
                ...
                freshness = 'stale'
            if age_sec < 60:
                last_friendly = 'Just now'
            elif age_sec < 3600:
                last_friendly = f'{int(age_sec // 60)} min ago'
            elif age_hours < 48:
                last_friendly = f'{age_hours:.1f}h ago'
            else:
                last_friendly = refreshed_at.strftime('%d %b %Y, %I:%M %p')
        except (TypeError, ValueError):
            last_friendly = str(last_refresh)[:19]

    history = []
    for item in (settings.get('refresh_history') or [])[:6]:
        at = item.get('at')
        stamp = _friendly_stamp(at)
        history.append({
            'at': stamp,
            'by': item.get('by') or 'Admin',
    ...
    sources = [
        {
            ...
            'hint': b['hint'],
            'count': b['count'],
            'size_label': _format_bytes(b.get('size_bytes') or 0),
        }
        for b in banks
    ]

    return render_template(
        'admin_refresh_data.html',
        sources=sources,
        total_records=total_records,
        last_refresh=last_refresh,
        last_refresh_friendly=last_friendly,
        last_refresh_by=last_by,
        age_hours=age_hours,
        freshness=freshness,
        history=history,
    )
...
def profile_form():
    ...
    if request.method == 'POST':
        ...
        if student:
            # Update existing — never store a password on the student profile document
...
def settings():
    ...
    secret_fields = (
        'MAIL_PASSWORD', 'GEMINI_API_KEY', 'TWILIO_ACCOUNT_SID',
    ...
    if request.method == 'POST':
        ...
        config_data['MAIL_USERNAME'] = request.form.get('MAIL_USERNAME') or config_data.get('MAIL_USERNAME', '')
        config_data['TWILIO_PHONE_NUMBER'] = request.form.get('TWILIO_PHONE_NUMBER') or config_data.get('TWILIO_PHONE_NUMBER', '')
        config_data['SENDGRID_FROM_EMAIL'] = request.form.get('SENDGRID_FROM_EMAIL') or config_data.get('SENDGRID_FROM_EMAIL', '')
        for field in secret_fields:
            new_val = (request.form.get(field) or '').strip()
            if new_val:
                config_data[field] = new_val
    ...
    for field in secret_fields:
        if safe_config.get(field):
            safe_config[field + '_configured'] = True
        safe_config[field] = ''
    return render_template('settings.html', config_data=safe_config)
...
def smtp_debug():
    """Admin-only diagnostic for SMTP config. Passwords are always masked."""
    if not session.get('logged_in') or session.get('role') != 'admin':
        return jsonify({'error': 'Unauthorized'}), 403
